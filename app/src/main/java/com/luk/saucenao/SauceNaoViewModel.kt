        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                when (data) {
                    ...
                    is Uri -> connection.data(
                        "file",
                        "image.png",
                        data.pngDataStream(context)
                    )
                ...
                when (response.statusCode()) {
                    200 -> {
                        ...
                        if (body.isNotEmpty()) {
                            _progressState.value = ProgressState.Success(body)
                        } else {
                            _progressState.value =
                                ProgressState.Error(R.string.error_cannot_load_results)
                            onError(R.string.error_cannot_load_results)
                        }
                    }

                    403 -> {
                        val errorType = if (response.headers()["cf-mitigated"] == "challenge")
                            R.string.error_cloudflare_challenge
                        else
                            R.string.error_cannot_load_results
                        _progressState.value = ProgressState.Error(errorType)
                        onError(errorType)
                    }

                    429 -> {
                        _progressState.value = ProgressState.Error(R.string.error_too_many_requests)
                        onError(R.string.error_too_many_requests)
                    }
                ...
                _progressState.value = ProgressState.Error(R.string.error_cannot_load_results)
                onError(R.string.error_cannot_load_results)
            }
        }
    }
